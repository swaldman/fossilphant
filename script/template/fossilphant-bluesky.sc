...
val selfUrl = Opts.option[String]("self-url", metavar="url", help="URL to which you'd like your display name and handle to link").orNone
val tagline = Opts.option[String]("tagline", help="Main tagline for the generated site").orNone
...
val userHandle = Opts.option[String]("handle", help="The DNS name by which the arcived account identifies itself.")
val images = Opts.option[String]("images", help="A directory containing all images referenced in the archive and downloaded from the account").map( os.Path( _, os.pwd ) )
...
val title = Opts.option[String]("title", help="Main title for the generated site").orNone
val archive = Opts.argument[String](metavar = "car-archive") //, help = "a car file exported by Bluesky")

case class Config( fc : FossilphantConfig, outDir : os.Path, userHandle : String, imageDir : os.Path )

val allOpts : Opts[Config] =
  (outDir, pageLength, selfUrl, tagline, theme, themeConfig, timezone, title, archive, userHandle, images).mapN { (od, pl, su, ta, tn, tc, tz, ti, a, uh, i) =>
    val fconfig = FossilphantConfig(
      pageLength = pl,
      newSelfUrl = su,
      mainTagline = ta,
      themeName = tn,
    ...
    val handle = uh.dropWhile( _ == '@' ) // we want a no-@ handle
    val imageDir = i
    Config(fconfig, outPath, handle, imageDir)
  }
...
val command = Command(name=s"${scriptName}", header="Generates a static site from a Bluesky archive")( allOpts )
